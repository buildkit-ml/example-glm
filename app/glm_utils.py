...
def fill_blanks_efficient(raw_texts: str, model, tokenizer, strategy, device, config=None):
    ...
    results = batch_filling_sequence(
        model,
        input_seqs,
        torch.cuda.LongTensor([input_seqs.shape[-1] for _ in range(input_seqs.shape[0])], device=device),
        strategy=strategy,
...
def post_processing_text(output_text, args, prompt_str_length):
...
def to_result(output, args, prompt_str_length, last_layer_embedding, top_logprobs, job_ids,
              working_directory=None, upload_token=""):
    ...
    for i in range(len(output)):
        ...
        if last_layer_embedding is not None:
            ...
            current_tensor_emb = last_layer_embedding[i]
            print(f"<to_result> job:{job_id} output shape: {current_tensor_emb.shape}")
            torch.save(current_tensor_emb, tensor_filename)
            with open(tensor_filename, "rb") as fp:
                ...
                item['embedding'] = res["filename"]
                os.remove(tensor_filename)

        print(f"<to_result> output{i}: {prompt_str_length[i]} / {len(output[i][0])}")
        choice = {
            "text": post_processing_text(output[i][0], args, prompt_str_length[i]),
            "index": 0,
